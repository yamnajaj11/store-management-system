    ...
    <div class="bg-white p-4 shadow-sm rounded-4 border">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0 text-primary fw-bold">
                <i class="bi bi-pencil-square me-2"></i> {{ __('products.edit_product') }}
            </h4>
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" class="needs-validation" novalidate>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" name="purchase_price" class="form-control" 
                                           value="{{ old('purchase_price', (int) $product->purchase_price) }}" required>
                                </div>

                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">{{ __('products.selling_price') }}</label>
                                    <input type="number" name="selling_price" class="form-control" 
                                           value="{{ old('selling_price', (int) $product->selling_price) }}" required>
                                </div>

                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">{{ __('products.stock') }}</label>
                                    <input type="number" name="stock" class="form-control" 
                                           value="{{ old('stock', (int) $product->stock) }}" required>
                                </div>
            ...
            <div class="mt-4 text-end">
                <button type="submit" class="btn btn-primary px-4">
                    <i class="bi bi-check-circle me-1"></i> {{ __('products.update') }}
                </button>
